	...
	*/
	...
	public function ucp_prefs_get_data($event)
	{
		...
		$event['data'] = array_merge(
			$event['data'], [
				'user_rtng_enable'          => $this->request->variable('user_rtng_enable', (int) $this->user->data['user_rt_enable']),
				'user_rtng_location'        => $this->request->variable('user_rtng_location', $this->user->data['user_rt_location']),
				'user_rtng_number'          => $this->request->variable('user_rtng_number', (int) $this->user->data['user_rt_number']),
				'user_rtng_sort_start_time' => $this->request->variable('user_rtng_sort_start_time', (int) $this->user->data['user_rt_sort_start_time']),
				'user_rtng_unread_only'     => $this->request->variable('user_rtng_unread_only', (int) $this->user->data['user_rt_unread_only']),
			]
		);
		...
		if (!$event['submit'] && $this->auth->acl_get('u_rt_view'))
		{
			...
			if ($this->auth->acl_get('u_rt_view') && ($this->auth->acl_get('u_rt_enable') || $event['data']['user_rtng_enable']) && ($this->auth->acl_get('u_rt_enable') || $this->auth->acl_get('u_rt_location') || $this->auth->acl_get('u_rt_sort_start_time') || $this->auth->acl_get('u_rt_unread_only')))
			{
				$template_vars += [
					'S_RTNG_SHOW' => true,
				];
	...
	public function ucp_prefs_set_data($event)
	{
		$event['sql_ary'] = array_merge(
			$event['sql_ary'], [
				'user_rt_enable'		  => $event['data']['user_rtng_enable'],
				'user_rt_location'		  => $event['data']['user_rtng_location'],
				'user_rt_number'		  => $event['data']['user_rtng_number'],
				'user_rt_sort_start_time' => $event['data']['user_rtng_sort_start_time'],
				'user_rt_unread_only'	  => $event['data']['user_rtng_unread_only'],
			]
		);
	}
